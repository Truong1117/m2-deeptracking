...
namespace Commercers\DeepTracking\Cron\Hellmann;

use Magento\Sales\Model\OrderFactory;
use Magento\Framework\Model\ResourceModel\Iterator;
use Magento\Framework\App\ResourceConnection;
use Commercers\DeepTracking\Model\HellmannLogFactory;
...
use Commercers\DeepTracking\Service\Api\Hellmann\ParcelTrack;

class Track {

	/**
	 * @var HellmannLogFactory
	 */
	...
	{
		if(!$this->_config->isHellmannEnabled()){
			return;
		}
		$daysBack = $this->_config->getHellmannDayBack();
		$orderPerRun = $this->_config->getHellmannCronLimit();
        $carrierTitle = $this->_config->getHellmannCarrierTitle();
		...
		$filterStatuses = explode(',',$this->_config->getHellmannOrderStatuses());
	...
	public function changeOrderStatus($args)
	{
		...
		if(!empty($log->getData())){
			...
			if( $orderStatus == HellmannStatus::HELLMANN_DELIVERED_SUCCESSFULLY){
				$log->delete();
				return $this;
			} else {
				$logData = [
		...
		usleep(500); // Prevent bulk requests to Hellmann API
	}
